...
voltage divider resistors R20 and R21 to be installed in ARPS. 
...
See the https://mirobo.tech/arps website for additional ARPS
resources, programming activities, and starter programs.
*/

// ARPS Educational Starter I/O devices
...
// ARPS optional analog input devices
...
1.  Let's use this program to learn how to process analog data from
    a temperature sensor or a voltage divider (to do this, your ARPS
...
4.  The analog temperature sensor used in the ARPS circuit has a
    ...
    For the MCP9700A temperature sensor shown in the ARPS schematic, 
...
5.  The values of the voltage divider resistors in the ARPS circuit
    ...
    shown in the ARPS schematic diagram, and a voltage input of 6V,
    ...
    ARPS. Knowing this, the ADC output value corresponding to any
    ...
    With a 6V input to ARPS, and the calculated voltage divider ratio
  ...
  volts = float(rawVolts) / 40;
    ...
    take actual voltage measurements, your ARPS and Arduino will 
    need to be powered using an external power supply connected to
    the DC inputs of the ARPS screw terminal strip. Do that now, and
    then upload the program to view the results. Does the voltage
    reading in the Serial Monitor match the input voltage?

6.  Whether or not your ARPS has its voltage divider resistors
...
1.  Create a program that uses the binToDec() function to convert
    ...
    digits in succession on as binary numbers on ARPS's LEDs.
...
3.  Analog input values above or below chosen thresholds can be used
    ...
    of state. One of the features of the ARPS circuit is its break-
...
5.  Analog inputs can be used to control analog output devices, either
    ...
    example, a potentiometer connected to one of ARPS's header pins
