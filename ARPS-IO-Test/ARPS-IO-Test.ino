...
by default, and also works on Arduino UNO R3.
...
const int SW5 = 3;      // circuit boards

const int LED2 = 5;     // ARPS LEDs (shared with motor outputs)
const int LED3 = 6;
const int LED4 = 9;
const int LED5 = 10;
...
// ARPS motor output pin definitions (redefines same I/O pins as LEDs)
...
// ARPS optional special purpose I/O pins
const int IR = 4;       // IR demodulator
...
const int LED6 = 7;     // Left floor sensor module IR LED
const int LED7 = 8;     // Right floor sensor module IR LED
...
void setup() {
  ...
  tone(BEEPER, 4000, 100);  // Say hello and print some messages

  Serial.println(F("Starting ARPS"));
...
void loop() {
  ...
  if(SW3State == LOW) {         // Only usable on UNO R4 family
    digitalWrite(LED3, HIGH);   // LED3 blinks on UNO R3
  }
  else {
