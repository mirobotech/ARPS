...
1.  Let's analyze this program! Program analysis is an important skill
    ...
    Install your ARPS circuit on top of your Arduino, and then connect
    ...
    pushbutton SW2 on your ARPS circuit while observing its LEDs.
...
2.  The first line of code in the program's main 'loop()' function is:
  ...
  if(SW2State == LOW) {
    ...
    and LOW correspond to on the Arduino UNO used with your ARPS?
...
6.  Comment-out the second 'SW5State = digitalRead(SW5);' statement --
    ...
    getting stuck, press and release the SW1 RESET pushbutton on ARPS
